...
Nous allons voir ici comment implémenter le déplacement vers la droite puis vers la gauche du personnage.
...
Vous pouvez téléverser votre programme vers votre console et maintenir le bouton droit, et constater que votre personnage va se déplacer jusqu'au bord droit de l'écran. Voilà une première partie du déplacement développé.
...
* si le personnage prend de l'élan, alors il réalise un "saut en cloche".
...
Bien ! Maintenant que nous avons apporter les modifications nécessaire au personnage, voyons l'implémentation du saut.
...
Dans un nouveau fichier, que nous pouvons nommer `PhysicsEngine.h`, ajoutons le code suivant :
```
...
Toujours dans la fonction `manageCommands`, il faut lorsque le joueur appuye sur le bouton A et que le personnage ne saute pas, alors on donne l'impulsion pour le saut, soit :
```
...
Vous pouvez désormais téléverser votre programme vers la console, et vous amuser à sauter partout. Il est à noter que si vous sauter sur le bord de l'écran alors le jeu est bloquer. Nous verrons dans la prochaine étape comment régler ce problème, ceci en implémentant la gravité / chute libre.

## Conclusion

Vous voici arrivé à la fin de cette deuxième étape : où nous avons ajouter les déplacements au personnage : avancer à gauche, avancer à droite et le saut.
